...
def get_video():
    try:
        ...
        # Your specific cookies to bypass "Bot" detection
        ...
        ydl_opts = {
            'format': '18/22/best', # Specifically looks for pre-merged files
            'quiet': True,
            'no_warnings': True,
            'skip_download': True,
            'nocheckcertificate': True,
            'extract_flat': 'in_playlist', # Speed hack: skips deep-scanning playlists
            'lazy_playlist': True,
            'socket_timeout': 5, # Kill connection if YouTube is too slow
            'http_headers': {
                'Cookie': my_cookies,
